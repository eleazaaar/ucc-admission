    ...
    if(isset($_POST['api_key'])) {
        ...
        if($api_key_value === $api_key){
            ...
            if ($datas == null) {
                ...
                $data = array(
                    'old_exam_schedule_id' => $_POST['old_exam_schedule_id'],
                    'old_seat_number' => $_POST['old_seat_number']
                );

                $stmt = $pdo->prepare("INSERT INTO exam_schedule_available SET exam_schedule_id = :old_exam_schedule_id, seat_number = :old_seat_number");
                $stmt->execute($data);

                // Count Available for New Schedule
                ...
                // Count Available for Old Schedule
                $stmt = $pdo->prepare("SELECT COUNT(*) as available_count FROM exam_schedule_available WHERE exam_schedule_id = ?");
                $stmt->execute([$_POST['old_exam_schedule_id']]);
                $datas = $stmt->fetchAll();
                ...
                $data = array(
                    'exam_schedule_id' => $_POST['old_exam_schedule_id'],
                    'available_count' => $available_count
                );
